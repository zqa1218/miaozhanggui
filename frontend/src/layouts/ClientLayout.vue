...
const storedMId = storage.get('mzg_client_mid', '')
const mId = ref(urlMId || storedMId || '')
...
function switchTab(tab) {
  if (tab === 'home') router.push('/studios?mId=' + mId.value)
  else router.push('/my-orders?mId=' + mId.value)
}

onMounted(async () => {
  if (urlMId) {
    mId.value = urlMId
    storage.set('mzg_client_mid', urlMId)
  }
  if (mId.value) {
    try {
