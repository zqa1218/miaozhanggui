    ...
    <div class="sa-body">
      ...
      <div class="sa-sidebar">
        <div :class="['sa-menu-item', { active: activeMenu === 'merchants' }]" @click="activeMenu = 'merchants'">
          <SvgIcon name="icon-dashboard" :size="18" />商家管理
        </div>
        <div :class="['sa-menu-item', { active: activeMenu === 'codes' }]" @click="activeMenu = 'codes'">
          <SvgIcon name="icon-ticket" :size="18" />邀请码生成
        </div>
      </div>
          ...
          <div v-if="newCodes.length > 0" class="new-codes-section" style="margin-top:20px;">
            ...
            <div class="code-card-grid">
              <div v-for="code in newCodes" :key="code" class="code-card">
                ...
                <el-button size="small" type="primary" @click="copyInviteLink(code)"><SvgIcon name="icon-copy" :size="14" />复制邀请链接</el-button>
              </div>
            </div>
          </div>
            ...
            <table class="data-table">
              ...
              <tbody>
                <tr v-for="c in allCodes" :key="c.code" :class="{ 'row-used': c.is_used }">
                  ...
                  <td>
                    <el-button v-if="!c.is_used" size="small" @click="copyInviteLink(c.code)"><SvgIcon name="icon-copy" :size="14" />复制</el-button>
                  </td>
                </tr>
              </tbody>
            </table>
...
.super-admin { min-height:100vh;min-height:100dvh; background:transparent; }
.sa-header {
  display:flex; justify-content:space-between; align-items:center;
  padding:16px 32px; background: var(--surface-solid); border-bottom:1px solid var(--border-subtle);
  box-shadow:0 2px 12px rgba(0,0,0,.04);
}
.sa-title { font-size:18px; font-weight:700; color:var(--text-1); }
.btn-logout { padding:8px 20px; border:1px solid var(--border-color); border-radius:20px; background: var(--surface-solid); cursor:pointer; font-size:13px; color:var(--text-3); }
.btn-logout:hover { background:var(--color-primary-tint); border-color:var(--color-primary); color: var(--color-primary-ink); }

.sa-body { display:flex; min-height:calc(100vh - 65px); }
.sa-sidebar {
  width:220px; background: var(--surface-solid); border-right:1px solid var(--border-subtle);
  padding:20px 0; flex-shrink:0;
}
.sa-menu-item {
  padding:14px 24px; font-size:14px; font-weight:600; color:var(--text-3);
  cursor:pointer; transition:all .15s;
}
.sa-menu-item:hover { color: var(--color-primary-ink); background:var(--color-primary-tint); }
.sa-menu-item.active { color: var(--color-primary-ink); background:var(--color-primary-tint); border-right:3px solid var(--color-primary); }
...
.kpi-value { font-size:56px; font-weight:800; color: var(--color-primary-ink); line-height:1.2; }
.kpi-label { font-size:15px; color:var(--text-3); margin-top:8px; font-weight:600; }

.section-title { font-size:16px; font-weight:700; color:var(--text-1); margin-bottom:4px; }

/* 表格 */
.table-wrap { overflow-x:auto; background: var(--surface-solid); border-radius:16px; box-shadow:0 2px 8px rgba(0,0,0,.03); }
.data-table { width:100%; border-collapse:collapse; font-size:13px; }
.data-table th { background:#FAFAFA; padding:12px 16px; text-align:left; font-weight:600; color:var(--text-3); border-bottom:1px solid var(--border-subtle); }
.data-table td { padding:12px 16px; border-bottom:1px solid #F8F6F3; color:var(--text-1); }
.data-table code { font-size:11px; background:#F8F6F3; padding:2px 6px; border-radius:4px; }
.code-tag { font-size:11px; background:var(--color-mint-light); color:var(--color-success-ink); padding:2px 8px; border-radius:4px; font-family:monospace; }

/* 邀请码卡片 */
.code-card-grid { display:flex; flex-wrap:wrap; gap:12px; margin-top:12px; }
.code-card {
  background: var(--surface-solid); border:1px solid var(--border-subtle); border-radius:16px;
  padding:16px 20px; display:flex; align-items:center; gap:16px;
  box-shadow:0 2px 8px rgba(0,0,0,.03);
}
.code-text { font-size:20px; font-weight:800; color:var(--text-1); font-family:'SF Mono','Cascadia Code',monospace; letter-spacing:2px; }

/* 状态徽章 */
.badge-used { font-size:11px; padding:3px 10px; border-radius:10px; background:rgba(180,180,190,.12); color:var(--text-3); font-weight:600; }
.badge-valid { font-size:11px; padding:3px 10px; border-radius:10px; background:rgba(168,216,185,.20); color:var(--color-success-ink); font-weight:600; }
.row-used { opacity:.60; }
