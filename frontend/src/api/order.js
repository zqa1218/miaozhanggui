...
export function createOrder(data) {
  return http.post('/create-order', data, 'client')
}

/** C端 V2：高精度闭合时间下单 */
export function createOrderV2(data) {
  return http.post('/create-order-v2', data, 'client')
}
...
export function preLockOrder(data) {
  return http.post('/bookings/pre-lock', data, 'client')
}

/** C端：我的订单列表 */
export function getMyOrders(params) {
  return http.get('/my-orders', params, 'client')
}

/** C端：支付订金 */
export function payDeposit(data) {
  return http.post('/pay-deposit', data, 'client')
}

/** C端：支付尾款 */
export function payFinal(data) {
  return http.post('/pay-final', data, 'client')
}

/** C端：订单详情 */
export function getOrderDetail(params) {
  return http.get('/order-detail', { params }, 'none')
}
...
export function requestReschedule(data) {
  return http.post('/order/request-reschedule', data, 'client')
}

/** C端：申请取消 */
export function requestCancel(data) {
  return http.post('/order/request-cancel', data, 'client')
}
