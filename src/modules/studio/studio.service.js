...
/** 客户端-精简列表 (mId 可选，city/keyword 筛选) */
async function getLiteList(filters = {}) {
  const rows = await repo.findAllPublic(filters);
  return Promise.all(rows.map(mapToLiteDTO));
}

/** 获取指定日期有空闲时段的工作室列表 */
async function getAvailable(date) {
  const [studios, booked, unavailable, openStudios, studiosWithAnyAvailability] = await Promise.all([
    repo.findActiveStudios(),
    repo.findBookedSlotsByDate(date),
    repo.findUnavailableSlotsByDate(date),
    repo.findStudiosOpenOnDate(date),
    knex('studio_availabilities').distinct('studio_id'),
  ]);

  const openOnDate = new Set(openStudios.map(r => r.studio_id));
  const hasAvailabilityRestriction = new Set(studiosWithAnyAvailability.map(r => r.studio_id));

  // 预计算每个工作室的已占用分钟数
  const bookedMins = {};
  for (const b of booked) {
    if (!bookedMins[b.studio_id]) bookedMins[b.studio_id] = 0;
    bookedMins[b.studio_id] += (toMin(b.end_time) - toMin(b.start_time));
  }

  // 不可用时段：每段 30 分钟
  const unavailableMins = {};
  for (const u of unavailable) {
    if (!unavailableMins[u.studio_id]) unavailableMins[u.studio_id] = 0;
    unavailableMins[u.studio_id] += 30;
  }

  const available = [];
  for (const s of studios) {
    // 可用日期校验
    if (hasAvailabilityRestriction.has(s.id) && !openOnDate.has(s.id)) {
      continue; // 有白名单但该日期不在其中
    }
    // 无白名单记录 → 默认全开放，继续

    const opMins = toMin(s.end_time) - toMin(s.start_time);
    if (opMins <= 0) continue;

    const used = (bookedMins[s.id] || 0) + (unavailableMins[s.id] || 0);
    if (used < opMins) {
      available.push({
        id: s.id,
        mId: s.m_id,
        ...
        city: s.city,
        singlePrice: Number(s.single_price) || null,
        packagePrice: Number(s.package_price) || null,
        depositRatio: s.deposit_ratio,
        isStyleEnabled: s.is_style_enabled,
        baseStartTime: fmtTime(s.start_time),
        baseEndTime: fmtTime(s.end_time),
      });
    }
  }
...
function toMin(t) {
  if (!t) return 0;
  const s = typeof t === 'string' ? t.slice(0, 5) : t;
...
async function getDistinctCities() {
  const rows = await repo.findDistinctCities();
...
module.exports = { getLiteList, getAvailable, getDistinctCities, getFullList, create, update, remove };
