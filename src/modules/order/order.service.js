...
async function payDeposit(orderNo, mId) {
  ...
  return knex.transaction(async (trx) => {
    ...
    await knex('orders').transacting(trx).where('order_no', orderNo)
      .update({ status: '已付定金', updated_at: knex.fn.now() });

    return { success: true, status: '已付定金' };
  });
}
...
async function getBookedTimesV2(mId, studioId, date, excludeOrderNo = '') {
  ...
  const slotQuery = knex('slot_bookings')
    .where({ m_id: mId, booking_date: date });
  if (studioId) slotQuery.where('studio_id', studioId);

  const [bookedRows, restRows, studio] = await Promise.all([
    slotQuery.select('start_time', 'end_time', 'lock_type', 'order_no', 'studio_id'),
...
async function getOrders(mId, filters = {}) {
  ...
  // ★ 诊断日志
  console.log('====== [BUG DIAGNOSIS] getOrders service 入口 ======');
  console.log('  mId:', mId);
  console.log('  filters:', JSON.stringify(filters));

  // ── 暴力无视过滤：直接查总表数量 ──
  const rawTotal = await knex('orders').count('* as c').first();
  console.log('====== [BUG DIAGNOSIS] 数据库 orders 总表物理行数:', rawTotal?.c);

  // ── 暴力无视过滤：该商户的所有订单 ──
  const merchantAll = await knex('orders').where('m_id', mId).count('* as c').first();
  console.log('====== [BUG DIAGNOSIS] 商户 ' + mId + ' 的所有订单数:', merchantAll?.c);

  // ── 强捞测试：无视过滤取前5条 ──
  const bypassOrders = await knex('orders').where('m_id', mId).select('order_no', 'status', 'service_status').limit(5);
  console.log('====== [BUG DIAGNOSIS] 强捞前5条:', JSON.stringify(bypassOrders));
  ...
  console.log('====== [BUG DIAGNOSIS] findMerchantPaginated 返回 total:', result?.total, 'rows:', result?.rows?.length);
  // ★ 错误隔离：单条数据损坏不影响整表
  const list = [];
...
async function confirmDepositPaid(orderNo, mId) {
  const order = await orderRepo.findByOrderNo(orderNo);
  if (!order || order.m_id !== mId) throw new AppError(ERROR_CODES.ORDER_NOT_FOUND, 404);
  validateTransition(PAYMENT_TRANSITIONS, order.payment_status, 'DEPOSIT_PAID', '支付状态');

  await knex('orders').where('order_no', orderNo).update({
    payment_status: 'DEPOSIT_PAID',
    updated_at: knex.fn.now(),
  });
  return { success: true, paymentStatus: 'DEPOSIT_PAID' };
}
...
async function markFulfilled(orderNo, mId) {
  const order = await orderRepo.findByOrderNo(orderNo);
  if (!order || order.m_id !== mId) throw new AppError(ERROR_CODES.ORDER_NOT_FOUND, 404);
  validateTransition(SERVICE_TRANSITIONS, order.service_status, 'FULFILLED', '服务状态');

  await knex('orders').where('order_no', orderNo).update({
    service_status: 'FULFILLED',
    updated_at: knex.fn.now(),
  });
  return { success: true, serviceStatus: 'FULFILLED' };
}
